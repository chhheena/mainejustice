...
$el.closest('ul.nav-tabs').find('.rl_tabs-toggle').attr('aria-selected',false);$el.attr('aria-selected',true);$el.tab('show');this.updateActiveClassesOnTabLinks($el);window.dispatchEvent(new Event('resize'));if(!slideshow){setTimeout(function(){$el[0].focus();},10);}},setScrollOnLoad:function($el){var self=this;if($el.parent().hasClass('in')||$el.parent().hasClass('active')){self.scrollOnLoad();return;}
