...
class Emails
{
	...
	private static function protectEmail($mailto, $text = '', $pre = '', $post = '')
	{
		...
		{
			return self::spoofEmailsInJSON($mailto, $text);
		}

		// In HTML
		if ($params->spoof == 2)
		{
			$text = JText::_($params->custom_text);
		}
		elseif ($text)
		{
			if ($params->spoof)
	...
	private static function protectEmailsInString(&$string)
	{
		...
		if ( ! empty($emails))
		{
			foreach ($emails as $email)
			{
				$protected = self::protectEmail($params->mode == 1 ? $email[1] : '', $email[1]);
				$string    = substr_replace($string, $protected, strpos($string, $email[0]), strlen($email[0]));
			}
		}
	...
	private static function spoofEmailsInFeeds($mailto, $text = '')
	{
		...
		return self::spoofEmailsInFormat(
			$mailto,
			$text,
			$params->protect_in_feeds == 2 ? $params->feed_text : ''
		);
	}

	private static function spoofEmailsInFormat($mailto, $text = '', $custom_text = '')
	{
	...
	private static function spoofEmailsInJSON($mailto, $text = '')
	{
		$params = Params::get();
		...
		return self::spoofEmailsInFormat(
			$mailto,
			$text,
			$params->protect_in_json == 2 ? $params->json_text : ''
		);
	}
	...
	private static function spoofEmailsInPDFs($mailto, $text = '')
	{
		...
		return self::spoofEmailsInFormat(
			$mailto,
			$text,
			$params->protect_in_pdfs == 2 ? $params->pdf_text : ''
		);
	}
}
