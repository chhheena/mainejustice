...
if(is_array($displayData)) {
	...
	$blog = (isset($displayData['item']) && $displayData['item']) ? $displayData['item'] : $displayData ;
} else {
	$type = '';
	$blog = $displayData;
}
...
$params = $blog->params;
$attribs = json_decode($blog->attribs);

$template = Factory::getApplication('site')->getTemplate(true);
$tplParams = $template->params;

$leading = (isset($blog->leading) && $blog->leading) ? 1 : 0;
$leading = (isset($blog->leading) && $blog->leading) ? 1 : 0;
...
elseif($type == 'link')
{
	$blog_list_image = $tplParams->get('blog_link_image', 'small');
}
else
{
...
<?php if($intro_image) : ?>
	<?php if ($params->get('link_titles') && $params->get('access-view')) : ?>
		<a href="<?php echo Route::_(ContentHelperRoute::getArticleRoute($blog->slug, $blog->catid, $blog->language)); ?>">
		<?php endif; ?>
...
<?php $images = json_decode($blog->images); ?>
<?php if (isset($images->image_intro) && !empty($images->image_intro)) : ?>
	<?php $imgfloat = empty($images->float_intro) ? $params->get('float_intro') : $images->float_intro; ?>
	<div class="article-intro-image float-<?php echo htmlspecialchars($imgfloat, ENT_COMPAT, 'UTF-8'); ?>">
		<?php if ($params->get('link_titles') && $params->get('access-view')) : ?>
			<a href="<?php echo Route::_(ContentHelperRoute::getArticleRoute($blog->slug, $blog->catid, $blog->language)); ?>"><img
