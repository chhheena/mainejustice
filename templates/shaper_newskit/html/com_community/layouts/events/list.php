...
if(!$isSearch && ($task == 'display' || !$task) && !$groupid && !$pageid){
?>
    ...
    <ul class="joms-list--card">
        ...
        <?php
            ...
            for ($i = 0; $i < count($events); $i++) {
                ...
                $isPageEvent = $event->type == CEventHelper::PAGE_TYPE;
                ...
                if($event->type == 'page' && $event->contentid){
                    ...
                    $pageName = $pageModel->getPageName($event->contentid);
                    $pageLink = CRoute::_( 'index.php?option=com_community&view=pages&task=viewpage&pageid='.$event->contentid);
                    $privatePage = $pageModel->needsApproval($event->contentid);
                }
                ...
                <li class="joms-list__item <?php echo $event->permission == 1 ? 'event-private' : 'event-public' ?>">
                    ...
                    <div class="joms-list__content">
                        ...
                        <p><?php echo CActivities::truncateComplex(strip_tags($event->summary), 120, true); ?></p>
                        ...
                        <div class="joms-list__body">
                            ...
                            <?php if ( !$isPastEvent && CEventHelper::seatsAvailable($event) && ( $event->allowinvite || $event->isAdmin($my->id) || $canInvite ) ) { ?>
                                <a href="javascript:" class="joms-button--secondary joms-button--small" onclick="joms.api.eventInvite('<?php echo $event->id; ?>', '<?php echo $isGroupEvent ? "group" : "" ?><?php echo $isPageEvent ? "page" : "" ?>')">
                                    <?php 
                                        if ($isGroupEvent) {
                                            echo JText::_('COM_COMMUNITY_EVENT_INVITE_GROUP_MEMBERS'); 
                                        } else if ($isPageEvent) {
                                            echo JText::_('COM_COMMUNITY_EVENT_INVITE_PAGE_MEMBERS'); 
                                        } else {
                                            echo JText::_('COM_COMMUNITY_TAB_INVITE'); 
                                        }
                                    ?>
                                </a>
                            <?php } ?>
                        ...
                        <ul class="joms-list--inline">
                            ...
                            <?php
                            if ($event->permission == 1 || (isset($privateGroup) && $privateGroup == 1) || (isset($privatePage) && $privatePage == 1)) { ?>
                                <li>
                                    <svg class="joms-icon" viewBox="0 0 16 16">
