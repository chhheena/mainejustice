...
<?php if ($this->item->state == 0 || strtotime($this->item->publish_up) > strtotime(JFactory::getDate()) || ((strtotime($this->item->publish_down) < strtotime(JFactory::getDate())) && $this->item->publish_down != JFactory::getDbo()->getNullDate())) :
    ...
    <div class="newskit-item-wrap">
        ...
        <div class="newskit-blog-content-wrap">
            ...
            <?php
            if ($this->item->item_type == 'leading') {
                echo '<div class="introtext">' . $this->item->introtext . '</div>';
            }
            ?>
            ...
            <?php
            if (($this->item->item_type != 'leading') && ($tpl_params->get('show_intro', 0))) {
                echo '<div class="introtext">' . $this->item->introtext . '</div>';
            }
            ?>
            ...
            <?php if ($this->item->state == 0 || strtotime($this->item->publish_up) > strtotime(JFactory::getDate()) || ((strtotime($this->item->publish_down) < strtotime(JFactory::getDate())) && $this->item->publish_down != JFactory::getDbo()->getNullDate())) :
                ?>
            </div> <!-- newskit-blog-content-wrap -->
        </div> <!-- newskit-item-wrap -->
    </div>
<?php endif; ?>
