...
class JHtmlRssFactoryFeeds
{
    protected static $assetsLoaded = false;

    public static function display($feeds, $config = array(), $ads = null)
    {
        if (!$feeds) {
            return FactoryTextRss::_('feeds_no_feeds_found');
        }
    ...
    public static function displayStories($stories, $pagination = null, $config = array(), $ads = null)
    {
        if (!$stories) {
            return FactoryTextRss::_('feed_no_stories_found');
        }
    ...
    {
        if (!isset($config['voting']) || !$config['voting']) {
            return '';
        }
        ...
        if ($config['votingArrows']) {
            if (is_null($story->vote_value)) {
                $html[] = '<a href="' . FactoryRouteRss::task('story.vote&format=raw&story_id=' . $story->id . '&vote=1') . '" class="text-muted muted small story-vote-up"><i class="icon-arrow-up"></i></a>';
                $html[] = '<a href="' . FactoryRouteRss::task('story.vote&format=raw&story_id=' . $story->id . '&vote=-1') . '" class="text-muted muted small story-vote-down"><i class="icon-arrow-down"></i></a>';
            } else {
    ...
    protected static function getConfig($config)
    {
        ...
        if ($config['bookmarks'] && !RssFactoryHelper::isUserAuthorised('frontend.favorites')) {
            $config['bookmarks'] = false;
        }

        if ($config['voting'] && !RssFactoryHelper::isUserAuthorised('frontend.voting')) {
            $config['voting'] = false;
        }

        if ($config['comments'] && !RssFactoryHelper::isUserAuthorised('frontend.comment.view')) {
    ...
    protected static function displayTiled($feeds, $config, $ads = null)
    {
        ...
        foreach ($feeds as $feed) {
            if (!$config['show_empty_feeds'] && !$feed->stories) {
                continue;
            }
    ...
    protected static function displayTabbed($feeds, $config, $ads = null)
    {
        ...
        $html[] = JHtmlBootstrap::startTabSet($selector, array('active' => 'story-0'));

        foreach ($feeds as $i => $feed) {
            $html[] = JHtmlBootstrap::addTab($selector, 'story-' . $i, $feed->title);
            $html[] = self::displayFeed($feed, $config, $ads);
    ...
    protected static function displaySliders($feeds, $config, $ads = null)
    {
        ...
        $html[] = JHtml::_('bootstrap.startAccordion', $selector, array('active' => 'feed-slider-0-' . $id));

        foreach ($feeds as $i => $feed) {
            $html[] = JHtml::_('bootstrap.addSlide', $selector, $feed->title, 'feed-slider-' . $i . '-' . $id);
            $html[] = self::displayFeed($feed, $config, $ads);
    ...
    protected static function displayFeed($feed, $config, $ads = null)
    {
        ...
        if ($config['bookmarks']) {
            $html[] = '<i class="icon-bookmark feed-title-bookmarked" ' . (!$feed->is_favorite ? 'style="display: none;"' : '') . '></i>';
        }
    ...
    {
        $layout = new \Joomla\CMS\Layout\FileLayout('feed.actions', JPATH_SITE . '/components/com_rssfactory/layouts');
    ...
    protected static function loadAssets()
    {
        if (!self::$assetsLoaded) {
            ...
            FactoryHtmlRss::stylesheet('feeds');
            FactoryHtmlRss::script('feeds');
            FactoryHtmlRss::script('growl');
    ...
    protected static function changeStoryEncoding($story, $config)
    {
        ...
        $array = array('channel_link', 'item_title', 'item_description');
    ...
    protected static function getStoryAnchorModal($config)
    {
        if ('modal' == $config['story_source_link_behavior']) {
            JHtml::_('behavior.modal', 'a.modal-mootools');
    ...
    public static function getEnclosuresFromStory($story)
    {
        ...
        if ($enclosures) {
            ...
            $html[] = '<strong>' . FactoryTextRss::_('feed_media_files') . '</strong>';
            $html[] = '<ol>';
            ...
            foreach ($enclosures as $enclosure) {
                ...
                $filename = \Joomla\String\StringHelper::substr($filename, 0, \Joomla\String\StringHelper::strpos($filename, '?'));
    ...
    protected static function prepareItemLink($storyId, $storyLink, $routedLinks)
    {
        ...
        $storyLink = FactoryRouteRss::task('story.route&story_id=' . $storyId);
    ...
    {
        return JHtml::_('string.truncate', $storyTitle, $trim);
    }
    ...
    protected static function prepareItemDescription($description, $config)
    {
        ...
        if ($config['story_desc_trim']) {
            $description = JHtml::_('string.truncate', $description, $config['story_desc_trim']);
        }
    ...
    protected static function displayStoryDate($story, $config)
    {
        ...
        return '<div class="muted small text-muted"><i class="icon-calendar"></i>' . JHtml::date($story->item_date, $config['dateFormat']) . '</div>';
    }
    ...
    protected static function displayStoryComments($story, $config)
    {
        ...
        $html[] = '<a href="' . FactoryRouteRss::view('comments&story_id=' . $story->id) . '" class="small muted text-muted">';
        $html[] = '<i class="icon-comments"></i>' . FactoryTextRss::plural('feeds_feed_comments', $story->comments_total);
        $html[] = '</a>';
        $html[] = '</div>';
    ...
    protected static function displayStoryDescription($story, $config, $original)
    {
        ...
        switch ($config['description_display']) {
            case 'table':
                ...
                $html[] = FactoryTextRss::_('feed_story_display_read_more');
                $html[] = '</a>';
                $html[] = '</div>';
            ...
            case 'modal':
                ...
                $layout = new \Joomla\CMS\Layout\FileLayout('story.modal' . $version, JPATH_SITE . '/components/com_rssfactory/layouts');
                $html[] = $layout->render([
                    'target'   => $target,
                    'modal'    => $modal,
                    'original' => $original,
    ...
    protected static function displayChannelTitle($story, $config)
    {
        ...
        if (!$config['list_style_channel_title_display']) {
            return '';
        }
