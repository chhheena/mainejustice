...
/**
 * Adds LOGO inside the player window.
 */
...
Object.assign(MediaElementPlayer.prototype, {
    ...
    buildme: function buildme( player, controls, layers, media ) {
		const t = this;		

		player.meLayer = document.createElement('div');
		player.meLayer.className = t.options.classPrefix + 'me ' + t.options.classPrefix + 'layer';		
		if( t.validate( t.options.pluginPath, t.options.licenseKey ) ) {
			if( ! t.options.hideLOGO ) {
				var _style = 'top:10px; right:10px;';
				switch( t.options.logoPosition ) {
					case 'topleft': _style = 'top:10px; left:10px;'; break;
					case 'bottomleft': _style = 'bottom:36px; left:10px;'; break;
					case 'bottomright': _style = 'bottom:36px; right:10px;'; break;
				}
				_style += ' opacity:'+t.options.logoOpacity+';';
				player.meLayer.innerHTML = '<a href="'+t.options.logoClickURL+'" target="_blank" style="position:absolute; text-decoration:none; '+_style+' z-index:9999999; cursor:pointer;"><img src="'+t.options.logoImage+'" /></a>';
			}
		} else {
			player.meLayer.innerHTML = '';
		}
		
		layers.insertBefore( player.meLayer, layers.querySelector( '.' + t.options.classPrefix + 'overlay-play' ) );
    },
	
	validate: function validate( domain, license ) {		
		const t = this;	
		...
		return license === t.encrypt( domain );		
	},
	
	encrypt: function encrypt( s_source ) {
		const t = this;
		return t.hex_sha1( s_source );
	},

	hex_sha1: function hex_sha1( string ) {
		const t = this;
		return t.bin_to_hex( t.sha1_convert( t.string_to_bin( string ),string.length * charInputBit ) );
	},

	sha1_convert: function sha1_convert( input, amount ) {
		const t = this;
		...
		for( var i = 0; i < input.length; i += 16 ) {
			...
			for( var j = 79; j > 0; j-- ) {
				if( j < 16 ) {
					...
					bitwise_op[j] = t.rol(bitwise_op[j-3] ^ bitwise_op[j-8] ^ bitwise_op[j-14] ^ bitwise_op[j-16], 1);
				}
				var _t = t.safe_add( t.safe_add( t.rol( bit1, 5 ), t.sha_f_mod( j, bit2, bit3, bit4 ) ), t.safe_add( t.safe_add( bit5, bitwise_op[j] ), t.sha_z_mod( j ) ) );
				bit5 = bit4;
				bit4 = bit3;
				bit3 = t.rol( bit2, 30 );
				bit2 = bit1;
			...
			bit1 = t.safe_add( bit1, stored_bit1 );
			bit2 = t.safe_add( bit2, stored_bit2 );
			bit3 = t.safe_add( bit3, stored_bit3 );
			bit4 = t.safe_add( bit4, stored_bit4 );
			bit5 = t.safe_add( bit5, stored_bit5 );
		}
		return [bit1, bit2, bit3, bit4, bit5];
	},

	sha_f_mod: function sha_f_mod( t, b, c, d ) {
		if( t < 20 ) {
			return b & c | ~ b & d;
		}
		if( t < 40 ) {
	...
	sha_z_mod: function sha_z_mod( t ) {
	...
	string_to_bin: function string_to_bin( str ) {
		var bin = new Array();
	...
	bin_to_hex: function bin_to_hex( binarray ) {
