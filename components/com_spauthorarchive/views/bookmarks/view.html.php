...
class SpauthorarchiveViewBookmarks extends HtmlView
{
	...
	function display($tpl = null) {
		...
		if (!$this->user->id) {
		    $login_url = Uri::root() . 'index.php?option=com_users&view=login&return=' .  urlencode(base64_encode( Uri::getInstance()->toString() ));
            $output  = '<div class="alert alert-warning">';
                $output  = '<div class="alert-message">';
                    $output .= '<span>' . Text::_('COM_SPAUTHORARCHIVE_NOT_LOGGED') .'</span>';
                $output .= '</div>';

                $output  .= '<div class="alert-login">';
                    $output .= '<a href="' . $login_url . '">' . Text::_('COM_SPAUTHORARCHIVE_CLICK_HEERE_TO_LOGIN') .'</a>';
                $output .= '</div>';
            $output .= '</div>';
            echo $output;
		...
		{
		    $existing_bookmarks = $model->getUserExistingBookmark($this->user->id);

            if (empty($existing_bookmarks->item_ids))
			{
                $output  = '<div class="alert alert-warning">';
                    $output .= '<span>' . Text::_('COM_SPAUTHORARCHIVE_NO_ITEMS_FOUND') .'</span>';
                $output .= '</div>';
		...
		{
			$url = (JVERSION >4 )? ContentHelperRoute::getArticleRoute($this->item->slug, $this->item->catid, $this->item->language) : RouteHelper::getArticleRoute($this->item->slug, $this->item->catid, $this->item->language);
