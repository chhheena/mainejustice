...
class MinitekWallModelMasonry extends JModelLegacy
{
	...
	function __construct()
	{
		...
		$this->masonry_pagination = $this->getMasonryPaginationLib();
	...
	public function getMasonryPaginationLib()
	{
		$options = new MinitekWallLibBaseMasonryPagination;
	...
	public function getAllResults($widgetID)
	{
		...
		$pageLimit = (int)$masonry_params['mas_page_limit'];
		$globalLimit = (int)$masonry_params['mas_global_limit'];
	...
	public function getAllResultsAjax($widgetID)
	{
		...
		if ($source_id == 'joomla')
		{
			$joomla_output = $this->getJoomlaResults($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $joomla_output);
		}

		// Custom Items
		else if ($source_id == 'custom')
		{
			$custom_output = $this->getCustomResults($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $custom_output);
		}

		// K2
		else if ($source_id == 'k2')
		{
			$k2_output = $this->getK2Results($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $k2_output);
		}

		// Jomsocial
		else if ($source_id == 'jomsocial')
		{
			$jomsocial_output = $this->getJomsocialResults($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $jomsocial_output);
		}

		// Virtuemart
		else if ($source_id == 'virtuemart')
		{
			$virtuemart_output = $this->getVirtuemartResults($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $virtuemart_output);
		}

		// Easyblog
		else if ($source_id == 'easyblog')
		{
			$easyblog_output = $this->getEasyblogResults($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $easyblog_output);
		}

		// Easysocial
		else if ($source_id == 'easysocial')
		{
			$easysocial_output = $this->getEasysocialResults($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $easysocial_output);
		}

		// Folder
		else if ($source_id == 'folder')
		{
			$folder_output = $this->getFolderResults($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $folder_output);
		}

		// RSS
		else if ($source_id == 'rss')
		{
			$rss_output = $this->getRSSResults($data_source, $startLimit, $pageLimit, $globalLimit);
			$output = array_merge($output, $rss_output);
		}

		// Empty output
		if (!$output || $output == '')
		{
			$output = 0; // =0 // for ajax purposes
		}
	...
	public function getEasyblogResults($data_source, $startLimit, $pageLimit, $globalLimit)
	{
		...
		// Easyblog Articles
		if ($easyblog_mode == 'eba') {
			$easyblog_articles = $this->easyblog_source->getEasyblogArticles('items', $data_source, $startLimit, $pageLimit, $globalLimit);

			if ($easyblog_articles) {
				$output = array_merge($output, $easyblog_articles);
			}
		}
