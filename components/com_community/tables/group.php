...
class CTableGroup extends JTable {
    ...
    public function getAvatar() {
        ...
        if($this->avatar == ''){
            ...
            if(isset($settings['group']['default-group-avatar'])) {
                $image = JUri::root() . str_replace(JPATH_ROOT, '',
                        COMMUNITY_PATH_ASSETS) . 'default-group-avatar.' . $settings['group']['default-group-avatar'];
    ...
    public function getThumbAvatar() {
        ...
        if($this->thumb == ''){
            ...
            if(isset($settings['group']['default-group-avatar'])) {
                $image = JUri::root() . str_replace(JPATH_ROOT, '',
                        COMMUNITY_PATH_ASSETS) . 'default-group-avatar-thumb.' . $settings['group']['default-group-avatar'];
    ...
    public function isMember($userid, $approved = 1) {
        ...
        $query .= ' AND ' . $db->quoteName('permissions') . '!=' . $db->Quote(COMMUNITY_GROUP_BANNED);
    ...
    public function getCover() {
        ...
        if (empty($this->cover)) {
            ...
            $scssTable = JTable::getInstance( 'Theme' , 'CommunityTable' );
            $settings = $scssTable->getByKey('settings');
            if(isset($settings['group']['default-cover-group'])){
                $image = JUri::root().str_replace(JPATH_ROOT,'',COMMUNITY_PATH_ASSETS).'default-cover-group.'.$settings['group']['default-cover-group'];
                return $image;
            }else{
                $this->cover = '';
            }
