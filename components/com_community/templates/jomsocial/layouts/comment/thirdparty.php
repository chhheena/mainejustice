...
<div class="joms-foo joms-touch">
    <div style="display:none"><?php echo $wallViewAll; ?></div>
    <?php
    //if the wall content is empty
    if(!$currentWallCount){ ?>
        <div class="joms-alert--info">
            <?php echo JText::_('COM_COMMUNITY_START_A_COMMENT'); ?>
        </div>
    <?php
    }
    ?>
    ...
    <?php if ($isLoggedIn) { ?>
        <?php if ($postCommentACL) { ?>
            <div class="joms-comment__reply joms-js--newcomment joms-js--newcomment-<?php echo $id; ?>" data-id="<?php echo $id; ?>">
                ...
                <span>
                    <button class="joms-button--comment joms-js--btn-send">Send</button>
                </span>
            </div>
        <?php } ?>
    <?php } else { ?>
        <a class="joms-button--neutral" href="javascript:" onclick="joms.api.login()"><?php echo JText::_('COM_COMMUNITY_LOGIN_TO_COMMENT') ?></a>
    <?php } ?>
    <script>
        (function( w ) {
