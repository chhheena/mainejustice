        ...
        <div class="joms-form__group">
            ...
            <div class="joms-js--pm-message" style="position:relative">
                ...
                <svg viewBox="0 0 16 16" class="joms-icon joms-icon--add" onclick="joms.view.comment.addAttachment(this);"
                        style="position:absolute;top:10px;<?php echo (JFactory::getLanguage()->isRTL()) ? 'left' : 'right'; ?>:10px">
                    <use xlink:href="#joms-icon-camera"></use>
                </svg>

                <?php if ($enableFileSharing) { ?>
                <svg viewBox="0 0 16 16" class="joms-icon joms-icon--add"
                     onclick="joms.view.comment.addAttachment(this, 'file', { type: 'message', max_file_size: '<?php echo $maxFileSize; ?>', exts: '<?php echo $fileExtensions ?>' });" style="position:absolute;top:10px;<?php echo (JFactory::getLanguage()->isRTL()) ? 'left' : 'right'; ?>:30px">
