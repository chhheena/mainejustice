...
class SppagebuilderAddonForm_builder extends SppagebuilderAddons
{
	...
	{ 
		...
		try
		{
			if ($mail->sendMail($sender_mail, $sender_name, $recipient, $email_subject_ajax, $email_body, $isHtmlMode, $cc, $bcc, $attachment, $replyToMail, $replyToName))
			{
				$output['status']  = true;
		...
		{
			$output['content'] = '<span class="sppb-text-danger">' . $e->getMessage() . '</span>';
		}
