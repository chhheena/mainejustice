...
class AddToMenu
{
	...
	public function __construct()
	{
		...
		if (is_array($params))
		{
			$params = json_encode($input->get('params', [], 'array'));
		}

		$params       = json_decode($params);
		$this->params = RL_Parameters::getModule('addtomenu', 1, $params);
	...
	public function render()
	{
		...
		{
			$option = $this->vars['extension'] ?? 'com_content';
		}
	...
	private function insertMenuItem(&$template)
	{
		...
		$item->parent_id = (int) ($menuitem[1] ?? 1);
		$item->access            = 1;
		$item->language          = '*';
		...
		$option = $this->vars['option'];
		$view   = $this->vars['view'] ?? $this->vars['task'] ?? '';
		...
		else
		{
			...
			if (file_exists($path))
			{
				$item->params = RL_Parameters::getObjectFromRegistry($item->params, $path);
			}
		}
	...
	private function renderHTML(&$template)
	{
		...
		else
		{
			...
			$item->alias = $template->dbselect->alias ?? $item->name;
		}
		...
		$default = $this->getDefaultValue('params:state', 1);

		$elements[] = $this->el(
			'JSTATUS',
			'published',
			'<fieldset id="published" class="radio btn-group">'
			. '<input type="radio" name="published" id="published1" value="1" ' . ($default ? 'checked="checked"' : '') . '>'
			. '<label for="published1">' . JText::_('JPUBLISHED') . '</label>'
			. '<input type="radio" name="published" id="published0" value="0" ' . (! $default ? 'checked="checked"' : '') . '>'
		...
		if (isset($template->extras->extra))
		{
			...
			foreach ($template->extras->extra as $element)
			{
				...
				$element->default = $this->getDefaultValue($element->default ?? '');
	...
	private function getDefaultValue($key, $default = '')
	{
		...
		$default_key = substr($key, 7);

		if ( ! isset($this->params->{'default_' . $default_key}))
		{
			return $key;
		}

		return $this->params->{'default_' . $default_key};
	}
	...
	private function getMenuItems($name)
	{
		...
		$default = $this->getDefaultValue('params:menuitem', $default);

		return JHtml::_('select.genericlist', $options, $name, null, 'value', 'text', $default);
	}
