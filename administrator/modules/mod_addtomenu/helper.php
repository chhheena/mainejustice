...
class ModAddToMenu
{
	...
	public function render()
	{
		...
		RL_Document::script('addtomenu/script.min.js', '6.7.0');
		RL_Document::stylesheet('addtomenu/style.min.css', '6.7.0');
				...
				if (isset($element->type))
				{
					switch ($element->type)
					{
						case 'radio':
							...
							$values       = $element->values->value ?? $element->values;
							$popup_height += 46 + (23 * (count($values) - 1));
							break;
						case 'textarea':
							$popup_height += 140;
							break;
		...
		if ($this->params->display_tooltip)
		{
			...
			$tip = '<strong>' . JText::_('ADDTOMENU') . '</strong><br>' . JText::_($template->name);
		}
		...
		if ($this->params->display_toolbar_button)
		{
			...
			$html[]  = '<a href="' . $link . '" class="btn btn-small addtomenu_link modal' . ($tip ? ' hasTooltip" title="' . $tip : '') . '"'
				. ' rel="{handler: \'iframe\', size: {x: Math.min(window.getSize().x-100, ' . $popup_width . '), y: Math.min(window.getSize().y-100, ' . $popup_height . ')}}">';
