...
class FactoryViewRss extends HtmlView
{
    protected
    ...
    protected function addToolbar()
    {
        ...
        foreach ($this->buttons as $type => $button) {
            ...
            switch ($type) {
                ...
                case 'delete':
                    JToolBarHelper::deleteList(FactoryTextRss::_('list_delete'), $this->getName() . '.delete');
                    break;
...
//                    JHtml::_('bootstrap.modal', 'collapseModal');
                    $title = FactoryTextRss::_($button[0]);
                ...
                default:
                    JToolBarHelper::custom($this->getName() . '.' . $button[0], $button[2], $button[2], FactoryTextRss::_($button[1]), $button[3]);
                    break;
            }
        }
    ...
    protected function setTitle()
    {
        if (isset($this->item) && $this->item) {
            if ($this->item->{$this->id}) {
                ...
                JToolBarHelper::title(FactoryTextRss::sprintf('view_title_edit_' . $this->getName(), $title, $this->item->{$this->id}));
            } else {
                JToolBarHelper::title(FactoryTextRss::_('view_title_new_' . $this->getName()));
            }
        } else {
            JToolBarHelper::title(FactoryTextRss::_('view_title_' . $this->getName()));
        }
    ...
    protected function loadAssets()
    {
        ...
        $prefix = JFactory::getApplication()->isClient('administrator') ? 'admin/' : '';
        ...
        foreach ($this->css as $css) {
            FactoryHtmlRss::stylesheet($css);
        }

        // Load js files.
        $this->js[] = $prefix . 'views/' . strtolower($this->getName());
        foreach ($this->js as $js) {
            FactoryHtmlRss::script($js);
        }

        // Register view html.
        foreach ($this->registerHtml as $html) {
            FactoryHtmlRss::registerHtml($html);
        }
    ...
    protected function addFilters()
    {
        ...
        foreach ($this->filters as $filter) {
            JHtmlSidebar::addFilter(
                FactoryTextRss::_('list_filter_title_' . $filter),
                'filter_' . $filter,
