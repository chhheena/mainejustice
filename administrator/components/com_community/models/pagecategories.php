...
class CommunityModelPageCategories extends JModelLegacy
{
	/**
	 * Configuration data
	 *
	...
	public function _buildQuery()
	{
		...
		$query	= 'SELECT a.*,'
			. 'COUNT(DISTINCT(b.id)) AS pagescount,'
			. 'COUNT(c.memberid) AS memberscount FROM '
			. $db->quoteName( '#__community_pages_category' ) . ' AS a '
			. 'LEFT JOIN ' . $db->quoteName( '#__community_pages' ) . ' AS b '
			. 'ON a.id=b.categoryid '
			. 'LEFT JOIN ' . $db->quoteName( '#__community_pages_members' ) . ' AS c '
			. 'ON b.id=c.pageid '
			. 'GROUP BY a.id';
		$query .= $orderby;
		return $query;
