...
class CommunityAdminModelPages extends JModelLegacy
{
	/**
	 * Configuration data
	 *
	...
	public function _buildQuery()
	{
		...
		$condition	= '';
		$ordering		= $mainframe->getUserStateFromRequest( "com_community.pages.filter_order",		'filter_order',		'a.name',	'cmd' );
		$orderDirection	= $mainframe->getUserStateFromRequest( "com_community.pages.filter_order_Dir",	'filter_order_Dir',	'',			'word' );
		...
		$query		= 'SELECT a.*, c.name AS username, COUNT(DISTINCT(b.memberid)) AS membercount FROM ' . $db->quoteName( '#__community_pages' ) . ' AS a '
	 ...
	 * Returns the Pages
	 *
	 * @return Array	Array of pages object
	 **/
	...
	public function getAllPages($orderBy='')
	{
		$db		= JFactory::getDBO();
		...
		$query	= "SELECT * FROM " . $db->quoteName( '#__community_pages').$sortQuery;
	...
	/**
	 * Returns the Pages Categories list
	 *
	 * @return Array An array of page category objects
	 **/
	public function &getCategories()
	{
		...
		$query	= 'SELECT * FROM ' . $db->quoteName( '#__community_pages_category');
		$db->setQuery( $query );
	...
	public function _getFields( $table = '#__community_pages' )
	{
		$result	= array();
		$db		= JFactory::getDBO();
	...
	public function getPagesbyInterval($interval = 'week')
	{
		$db = $this->getDBO();
		...
		{
			case 'week':
				$query = 'SELECT DATE('.$db->quoteName('created').') as created, COUNT('.$db->quoteName('created').') as count FROM '.$db->quoteName('#__community_pages')
	...
	/**
	 * Check if the given page name exist.
	...
	/**
	 * Retrieves the most active page throughout the site.
	 * @param   none
	 *
	 * @return  CTablePage The most active page table object.
	 **/
	public function getMostActivePage()
	{
		$db		= $this->getDBO();

		$query	= 'SELECT '.$db->quoteName('cid').' FROM '.$db->quoteName('#__community_activities')
				. ' WHERE '.$db->quoteName('app').' LIKE ' . $db->Quote( 'pages%' ).''
		...
		// return a null object if there is no page yet
		if( $id )
			return $page;
		else
			return null;
