...
function handle_side_menu($) {
    ...
    $("#sidebar-collapse").on(ace.click_event, function() {
        ...
        $(this).find('[class*="icon-"]:eq(0)').toggleClass("js-icon-double-angle-right");
    ...
    $(".nav-list .js-menu-dropdown-toggle").on(ace.click_event, function(g) {
        var $item = $(this);
        var $li = $item.parent();

        $li.toggleClass('open');
        $item.next().stop().slideToggle(200);
        
        $('.nav-list > li').not($li).each(function(idx, el) {
            var $el = $(el);
...
function widget_boxes($) {
    $(".page-content").delegate(".widget-toolbar > [data-action]", "click", function(k) {
        ...
        if (l == "collapse") {
            ...
            var b = "js-icon-" + e[1] + "-down";
            var f = "js-icon-" + e[1] + "-up";
