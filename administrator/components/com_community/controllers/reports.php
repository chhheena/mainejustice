...
class CommunityControllerReports extends CommunityController
{
	...
	public function removeReport()
	{
		...
		foreach( $ids as $id )
		{
			...
			if( !$row->deleteChilds() || !$row->delete( $id ) )
			{
				...
				return $this->setRedirect( 'index.php?option=com_community&view=reports' , $message, 'error');

			}
		}
		$message	= JText::sprintf( 'COM_COMMUNITY_REPORT_SUCCESSFULLY_REMOVE_COUNT' , $count );
		$this->setRedirect( 'index.php?option=com_community&view=reports' , $message, 'message' );
	}
	...
	public function ajaxPerformAction( $actionId, $ignore = 0 )
	{
		...
		// ask for delete if reported item not found or deleted
		if ($output == false) {
			$output = JText::_('COM_COMMUNITY_REPORTS_NOT_FOUND');
			$actions = '<input type="button" class="btn btn-mini pull-left" onclick="jQuery(\'input[type=checkbox]\').each(function(){ this.checked = this.value == ' . $action->reportid . '; }); Joomla.submitbutton(\'removeReport\');" value="' . JText::_('COM_COMMUNITY_YES') . '"/>';
			$actions .= '<input type="button" class="btn btn-inverse btn-mini pull-left" onclick="cWindowHide();" value="' . JText::_('COM_COMMUNITY_NO') . '"/>';
			$objResponse->addAssign( 'cWindowContent' , 'innerHTML' , $output);
			$objResponse->addScriptCall('cWindowActions', $actions);
	...
	public function purgeProcessed()
	{
		...
		$mainframe->redirect( 'index.php?option=com_community&view=reports' );
	}
}
