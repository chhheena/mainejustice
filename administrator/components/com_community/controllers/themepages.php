...
class CommunityControllerThemepages extends CommunityController
{
    ...
    public function apply()
    {
        ...
        if (strtoupper($jinput->getMethod()) != 'POST') {
            return $this->setRedirect('index.php?option=com_community&view=themepages', JText::_( 'COM_COMMUNITY_PERMISSION_DENIED' ) , 'error');
        }
        ...
        foreach($images as $key => $image) {
            if (!empty($image['tmp_name']) && isset($image['name']) && !empty($image['name'])) {
                ...
                if (!in_array($ext, array('jpg', 'jpeg', 'png')) || ($image['type'] != 'image/png' && $image['type'] != 'image/jpeg')) {
                    return $this->setRedirect('index.php?option=com_community&view=themepages', JText::_('COM_COMMUNITY_THEME_IMAGE_ERROR'), 'error');
                }
        ...
        return $this->setRedirect('index.php?option=com_community&view=themepages', $message, 'message');
    }
}