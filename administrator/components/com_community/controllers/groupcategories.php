	...
	public function removecategory()
	{
		...
		foreach( $ids as $id )
		{
			if(!$row->delete( $id ))
			{
				...
				return $this->setRedirect( 'index.php?option=com_community&view=groupcategories' , $message , 'error');
			}
		}
		$message	= JText::sprintf( 'COM_COMMUNITY_CATEGORY_SUCCESSFULLY_REMOVED_COUNT' , $count );
		$this->cacheClean(array(COMMUNITY_CACHE_TAG_GROUPS_CAT));
		$this->setRedirect( 'index.php?option=com_community&view=groupcategories' , $message ,'message' );
	}
}