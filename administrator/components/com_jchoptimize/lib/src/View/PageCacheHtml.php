...
{
    public function renderStatefulElements(Registry $state)
    {
        //Generate HTML for Search input box
        //language=HTML
        $searchInput = '<input type="text" name="filter[search]" id="filter_search" value="' . $state->get('filter_search') . '" class="form-control js-stools-field-search" aria-describedby="filter_search-desc" placeholder="Search by URL" inputmode="search">';
        ...
        $fullOrderingState = $state->get('list_fullordering', null);
    ...
    private function selectListGenerator($state, $type, $options, $value, $defaultValue = null)
    {
    ...
    private function getIcon($orderBy, $state)
    {
        ...
        list($fullordering, $direction) = \explode(' ', $state);
    ...
    private function getIconJ3($orderBy, $state)
    {
        if (!$state) {
            return '';
        }
