...
class MultiSelectItems implements Serializable
{
    ...
    {
        return $this->fileUtils->prepareForDisplay($folder);
    }
    public function prepareFileValues(string $file) : string
    {
        return $this->fileUtils->prepareForDisplay($file);
    }
    public function prepareClassValues($class) : string
    {
        return $this->fileUtils->prepareForDisplay($class);
    }
    /**
    ...
    public function generateAdminLinks(string $html, string $css, bool $bCssJsOnly) : array
    {
        ...
        $params->set('pro_remove_unused_js_enable', '0');
        try {
            ...
            $oHtmlParser->addExclude(Parser::HTML_COMMENT());
            $oHtmlParser->addExclude(Parser::HTML_ELEMENTS(['script', 'noscript', 'textarea']));
        ...
        JCH_DEBUG ? Profiler::stop('GenerateAdminLinks', \true) : null;
        return $aLinks;
    }
    /**
     *
    ...
    {
        if ($type == 'lazyload') {
            $aFieldOptions = $this->getLazyLoad($group);
            $group = 'file';
        } elseif ($type == 'images') {
            $group = 'file';
            ...
            $aFieldOptions = $this->getOptions($type, $group . 's');
        }
        $aOptions = array();
        $oParams = $this->params;
        $aExcludes = Helper::getArray($oParams->get($excludeParams, array()));
        foreach ($aExcludes as $sExclude) {
            $aOptions[$sExclude] = $this->{'prepare' . \ucfirst($group) . 'Values'}($sExclude);
        }
            ...
            if (!empty($aLinks['lazyload'])) {
                foreach ($aLinks['lazyload'] as $sImage) {
                    if ($group == 'folder') {
                        $regex = '#(?<!/)/[^/\\n]++$|(?<=^)[^/.\\n]++$#';
                        $i = 0;
                        $sImage = $this->fileUtils->prepareForDisplay($sImage, \true, \false);
                        $folder = \preg_replace($regex, '', $sImage);
                        while (\preg_match($regex, $folder)) {
                            $aFieldOptions[$folder] = $this->fileUtils->prepareForDisplay($folder, \true);
                        ...
                        $sImage = $this->fileUtils->prepareForDisplay($sImage, \true, \false);
                        $aFieldOptions[$sImage] = $this->fileUtils->prepareForDisplay($sImage, \true);
                    }
                }
            }
        } elseif ($group == 'class') {
    ...
    protected function getImages(string $action = 'exclude') : array
    {
        ...
        if (!empty($aLinks['images'][$action])) {
            foreach ($aLinks['images'][$action] as $sImage) {
                ...
                $aOptions[$sImage] = $this->fileUtils->prepareForDisplay($sImage);
            }
        }
     ...
     * @return array
     */
    protected function getOptions(string $type, string $exclude = 'files') : array
    {
        $aLinks = $this->links;
        $aOptions = array();
        if (!empty($aLinks[$type][0])) {
            foreach ($aLinks[$type][0] as $aLink) {
                if (isset($aLink['url']) && $aLink['url'] != '') {
                    if ($exclude == 'files') {
                        $file = $this->fileUtils->prepareForDisplay($aLink['url'], \true, \false);
                        $aOptions[$file] = $this->fileUtils->prepareForDisplay($file, \true, \true);
                    } elseif ($exclude == 'extensions') {
    ...
    public function prepareExtensionValues(string $url, bool $return = \true)
    {
        ...
        $host = $host == '' ? $oUri->toString(array('host')) : $host;
