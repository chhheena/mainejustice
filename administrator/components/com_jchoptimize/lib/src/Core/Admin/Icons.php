...
HTML;
    ...
    public function compileAutoSettingsIcons($settings) : array
    {
        ...
        for ($i = 0; $i < \count($settings); $i++) {
            ...
            $buttons[$i]['script'] = 'onclick="jchPlatform.applyAutoSettings(\'' . $settings[$i]['setting'] . '\', \'' . $id . '\'); return false;"';
            $buttons[$i]['id'] = $id;
...
HTML;
    ...
    public function compileUtilityIcons($aUtilities) : array
    {
        $aIcons = [];
        $i = 0;
        foreach ($aUtilities as $aUtility) {
            ...
            $aIcons[$i]['id'] = $this->generateIdFromName($aUtility['name']);
    ...
    {
        return [['name' => 'Add Image Attributes', 'setting' => $setting = 'img_attributes_enable', 'icon' => 'img_attributes.png', 'enabled' => $this->params->get($setting, '0'), 'tooltip' => Utility::translate('Adds \'height\' and/or \'width\' attributes to &lt:img&gt;\'s, if missing, to reduce CLS.')], ['name' => 'Sprite Generator', 'setting' => $setting = 'csg_enable', 'icon' => 'sprite_gen.png', 'enabled' => $this->params->get($setting, '0'), 'tooltip' => Utility::translate('Combines select background images into a sprite.')], ['name' => 'Http/2 Push', 'setting' => $setting = 'http2_push_enable', 'icon' => 'http2_push.png', 'enabled' => $this->params->get($setting, '0'), 'tooltip' => Utility::translate('Preloads critical assets using the http/2 protocol to improve LCP.')], ['name' => 'Lazy Load Images', 'setting' => $setting = 'lazyload_enable', 'icon' => 'lazyload.png', 'enabled' => $this->params->get($setting, '0'), 'tooltip' => Utility::translate('Defer images that fall below the fold.')], ['name' => 'Optimize CSS Delivery', 'setting' => $setting = 'optimizeCssDelivery_enable', 'icon' => 'optimize_css_delivery.png', 'enabled' => $this->params->get($setting, '0'), 'tooltip' => Utility::translate('Eliminates CSS render-blocking')], ['name' => 'Optimize Google Fonts', 'setting' => $setting = 'pro_optimize_gfont_enable', 'icon' => 'optimize_gfont.png', 'enabled' => $this->params->get($setting, '0'), 'proonly' => \true, 'tooltip' => Utility::translate('Optimizes the loading of Google fonts.')], ['name' => 'CDN', 'setting' => $setting = 'cookielessdomain_enable', 'icon' => 'cdn.png', 'enabled' => $this->params->get($setting, '0'), 'tooltip' => Utility::translate('Loads static assets from a CDN server. Requires the CDN domain(s) to be configured on the Configuration tab.')], ['name' => 'Smart Combine', 'setting' => $setting = 'pro_smart_combine', 'icon' => 'smart_combine.png', 'enabled' => $this->params->get($setting, '0'), 'proonly' => \true, 'tooltip' => Utility::translate('Intelligently combines files in a number of smaller files, instead of one large file for better http2 delivery.')], ['name' => 'Page Cache', 'setting' => 'integrated_page_cache_enable', 'icon' => 'cache.png', 'enabled' => Utility::isPageCacheEnabled($this->params), 'tooltip' => Utility::translate('Toggles on/off the Page Cache feature.')]];
    }
    ...
    {
        return [['name' => 'Remove Unused CSS', 'setting' => $setting = 'pro_remove_unused_css', 'icon' => 'remove_unused_css.png', 'enabled' => $this->params->get($setting, '0'), 'proonly' => \true, 'tooltip' => Utility::translate('Loads only the critical CSS required for rendering the page above the fold until user interacts with the page. Requires Optimize CSS Delivery to be enabled and may need the CSS Dynamic Selectors setting to be configured to work properly.')], ['name' => 'Remove Unused Javascript', 'setting' => $setting = 'pro_remove_unused_js_enable', 'icon' => 'remove_unused_js.png', 'enabled' => $this->params->get($setting, '0'), 'proonly' => \true, 'tooltip' => Utility::translate('Will defer the loading of javascript until the user interacts with the page to improve performance affected by unused javascript. If your site uses javascript to perform the initial render you may need to \'exclude\' these critical javascript. These will be bundled together, preloaded and loaded asynchronously.')], ['name' => 'Reduce DOM', 'setting' => $setting = 'pro_reduce_dom', 'icon' => 'reduce_dom.png', 'enabled' => $this->params->get($setting, '0'), 'proonly' => \true, 'tooltip' => Utility::translate('\'Defers\' the loading of some HTML block elements to speed up page rendering.')]];
    }
    public function compileToggleFeaturesIcons($settings) : array
    {
        ...
        for ($i = 0; $i < \count($settings); $i++) {
            ...
            $script = <<<JS
onclick="jchPlatform.toggleSetting('{$settings[$i]['setting']}', '{$id}'); return false;"
JS;
            $buttons[$i]['link'] = '';
            $buttons[$i]['icon'] = $settings[$i]['icon'];
