...
class Controller implements LoggerAwareInterface, ContainerAwareInterface
{
    ...
    public function __construct(Registry $params, LoggerInterface $logger)
    {
        ...
        $this->options['is-transparent'] = $this->options['use-transparency'] == 'on' && \in_array($this->options['image-output'], ['GIF', 'PNG']);
    ...
    public function CreateSprite($aFilePaths, $returnValues = \false)
    {
        ...
        foreach ($aFilePaths as $sFile) {
            ...
            $aCdns = \array_keys($cdn->getCdnDomains());
            $sFilePath = \str_replace($aCdns, '', $sFile);
            /** @var FileUtils $fileUtils */
            $fileUtils = $this->container->get(FileUtils::class);
            $sFilePath = $fileUtils->getPath($sFilePath);
            $bFileExists = \true;
            ...
            if ($bFileExists && !empty($sFileClass) && \in_array(\strtoupper($aPathParts['extension']), $this->imageTypes) && \in_array($iImageType, array(\IMAGETYPE_GIF, \IMAGETYPE_JPEG, \IMAGETYPE_PNG)) && \substr($sFileBaseName, 0, 1) != '.' && $iWidth < 50 && $iHeight < 50 && $iWidth > 0 && $iHeight > 0) {
        ...
        if (\count($aFilesInfo) > 1) {
            ...
            try {
                ...
                $this->bTransparent = !empty($this->options['use-transparency']) && \in_array($this->options['image-output'], array('GIF', 'PNG'));
