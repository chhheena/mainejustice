...
class Updates implements DatabaseModelInterface, StatefulModelInterface
{
    ...
    {
        $db = $this->db;
        $query = $db->getQuery(\true)->select($db->qn('update_site_id'))->from($db->qn('#__update_sites_extensions'))->where($db->qn('extension_id') . ' = ' . $db->q($this->extension_id));
        $db->setQuery($query);
        try {
            $ret = $db->loadColumn(0);
        } catch (Exception $e) {
    ...
    private function fixUpdates(int $updateSiteId, ?string $extraQuery) : bool
    {
        ...
        try {
            $db = $this->db;
            $query = $db->getQuery(\true)->select([$db->qn('update_id'), $db->qn('extra_query')])->from($db->qn('#__updates'))->where($db->qn('update_site_id') . ' = ' . $db->q($updateSiteId));
    ...
    public function adoptMyExtensions() : void
    {
        ...
        $db = $this->db;
        $query = $db->getQuery(\true)->update($db->quoteName('#__extensions'))->set($db->qn('package_id') . ' = ' . $db->q($newPackageId))->where($db->qn('extension_id') . 'IN(' . \implode(', ', \array_map([$db, 'q'], $extensionIDs)) . ')');
        $db->setQuery($query)->execute();
    }
    /**
    ...
    public function getUpdates(bool $force = \false) : array
    {
        ...
        $query = $db->getQuery(\true)->select('*')->from($db->qn('#__updates'))->where($db->qn('extension_id') . ' = ' . $db->q($this->extension_id));
        $db->setQuery($query);
