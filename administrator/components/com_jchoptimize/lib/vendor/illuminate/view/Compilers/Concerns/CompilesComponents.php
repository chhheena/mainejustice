...
trait CompilesComponents
{
    ...
    {
        [$component, $alias, $data] = \strpos($expression, ',') !== \false ? \array_map('trim', \explode(',', \trim($expression, '()'), 3)) + ['', '', ''] : [\trim($expression, '()'), '', ''];
    ...
    {
        return \implode("\n", ['<?php if (isset($component)) { $__componentOriginal' . $hash . ' = $component; } ?>', '<?php $component = $__env->getContainer()->make(' . Str::finish($component, '::class') . ', ' . ($data ?: '[]') . '); ?>', '<?php $component->withName(' . $alias . '); ?>', '<?php if ($component->shouldRender()): ?>', '<?php $__env->startComponent($component->resolveView(), $component->data()); ?>']);
    }
    /**
    ...
    protected function compileEndComponent()
    {
        ...
        return \implode("\n", ['<?php if (isset($__componentOriginal' . $hash . ')): ?>', '<?php $component = $__componentOriginal' . $hash . '; ?>', '<?php unset($__componentOriginal' . $hash . '); ?>', '<?php endif; ?>', '<?php echo $__env->renderComponent(); ?>']);
    }
    /**
     * Compile the end-component statements into valid PHP.
     *
    ...
    {
        return \is_string($value) || \is_object($value) && \method_exists($value, '__toString') ? e($value) : $value;
    }
}
