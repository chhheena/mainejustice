...
use stdClass;
use Traversable;
class WinCache extends AbstractAdapter implements AvailableSpaceCapableInterface, FlushableInterface, TotalSpaceCapableInterface
{
    /**
     * Constructor
     *
    ...
    protected function internalGetMetadata(&$normalizedKey)
    {
        ...
        $prefix = $namespace === '' ? '' : $namespace . $options->getNamespaceSeparator();
        $internalKey = $prefix . $normalizedKey;
        $info = \wincache_ucache_info(\true, $internalKey);
    ...
    protected function internalAddItem(&$normalizedKey, &$value)
    {
        ...
        if (!\wincache_ucache_add($internalKey, $value, $ttl)) {
            $type = \is_object($value) ? \get_class($value) : \gettype($value);
            throw new Exception\RuntimeException("wincache_ucache_add('{$internalKey}', <{$type}>, {$ttl}) failed");
        }
        return \true;
    }
    /**
    ...
    protected function internalGetCapabilities()
    {
        if ($this->capabilities === null) {
            $marker = new stdClass();
            $capabilities = new Capabilities($this, $marker, ['supportedDatatypes' => ['NULL' => \true, 'boolean' => \true, 'integer' => \true, 'double' => \true, 'string' => \true, 'array' => \true, 'object' => 'object', 'resource' => \false], 'supportedMetadata' => ['internal_key', 'ttl', 'hits', 'size'], 'minTtl' => 1, 'maxTtl' => 0, 'staticTtl' => \true, 'ttlPrecision' => 1, 'useRequestTime' => \false, 'namespaceIsPrefix' => \true, 'namespaceSeparator' => $this->getOptions()->getNamespaceSeparator()]);
    ...
    protected function normalizeMetadata(array &$metadata)
    {
        ...
        unset($metadata['key_name'], $metadata['hitcount'], $metadata['ttl_seconds'], $metadata['value_size']);
    }
}
