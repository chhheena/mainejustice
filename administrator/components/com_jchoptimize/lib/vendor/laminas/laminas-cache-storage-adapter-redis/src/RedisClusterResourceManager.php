...
use _JchOptimizeVendor\Laminas\Cache\Storage\Plugin\Serializer;
...
final class RedisClusterResourceManager implements RedisClusterResourceManagerInterface
{
    ...
    {
        if ($options->hasName()) {
            return $this->createRedisResourceFromName($options->getName(), $options->getTimeout(), $options->getReadTimeout(), $options->isPersistent(), $options->getPassword());
        }
        $password = $options->getPassword();
        if ($password === '') {
            $password = null;
        }
        return new RedisClusterFromExtension(null, $options->getSeeds(), $options->getTimeout(), $options->getReadTimeout(), $options->isPersistent(), $password);
    }
    /**
     * @psalm-param non-empty-string $name
     */
    private function createRedisResourceFromName(string $name, float $fallbackTimeout, float $fallbackReadTimeout, bool $persistent, string $fallbackPassword) : RedisClusterFromExtension
    {
        ...
        return new RedisClusterFromExtension(null, $seeds, $timeout, $readTimeout, $persistent, $password);
    }
    /**
     * @psalm-param array<positive-int,mixed> $options
     */
    private function applyLibraryOptions(RedisClusterFromExtension $resource, array $options) : RedisClusterFromExtension
    {
    ...
    public function hasSerializationSupport(PluginCapableInterface $adapter) : bool
    {
        ...
        $serializer = $options->getLibOption(RedisClusterFromExtension::OPT_SERIALIZER, RedisClusterFromExtension::SERIALIZER_NONE);
