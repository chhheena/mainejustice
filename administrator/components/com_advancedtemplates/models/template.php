...
use Joomla\CMS\MVC\Model\FormModel as JModelForm;
...
class AdvancedTemplatesModelTemplate extends JModelForm
{
	...
	public function copy()
	{
		...
		if (JFolder::exists($toPath) && ! JFolder::delete($toPath))
		{
	...
	public function copyFile($newName, $location, $file)
	{
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateFileCreate',
			['com_advancedtemplates.template', $template, $newName . '.' . $ext]
		);
	...
	public function createFile($name, $type, $location)
	{
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateFileCreate',
			['com_advancedtemplates.template', $template, $location . '/' . $name . '.' . $type]
		);
	...
	public function createFolder($name, $location)
	{
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateFileCreate',
			['com_advancedtemplates.template', $template, $name]
		);
	...
	public function createOverride($override)
	{
		...
		if ( ! JFolder::exists($htmlPath) && ! JFolder::create($htmlPath))
		{
	...
	public function cropImage($file, $w, $h, $x, $y)
	{
		...
		try
		{
			...
			JEventDispatcher::getInstance()->trigger(
				'onAfterTemplateFileUpdate',
				['com_advancedtemplates.template', $template, $relPath]
			);
	...
	public function deleteFile($file)
	{
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateFileDelete',
			['com_advancedtemplates.template', $template, urldecode(base64_decode($file))]
		);
	...
	public function deleteFolder($location)
	{
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateFileDelete',
			['com_advancedtemplates.template', $template, $location]
		);
	...
	public function getForm($data = [], $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_advancedtemplates.source', 'source', ['control' => 'jform', 'load_data' => $loadData]);
	...
	public function renameFile($file, $name)
	{
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateFileUpdate',
			['com_advancedtemplates.template', $template, $newName]
		);
	...
	public function save($data)
	{
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateFileUpdate',
			['com_advancedtemplates.template', $template, $fileName]
		);
	...
	public function uploadFile($file, $location)
	{
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateFileCreate',
			['com_advancedtemplates.template', $template, $url]
		);
	...
	protected function loadFormData()
	{
		...
		$this->preprocessData('com_advancedtemplates.source', $data);
