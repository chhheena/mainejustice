 ...
 */
class AdvancedTemplatesControllerTemplate extends JController
{
	/**
	...
	{
		$this->setRedirect(JRoute::_('index.php?option=com_advancedtemplates&view=templates', false));
	}
	...
	public function close()
	{
		...
		$id   = $app->input->get('id');
		$url  = 'index.php?option=com_advancedtemplates&view=template&id=' . $id . '&file=' . $file;
	...
	public function copy()
	{
		...
		$this->setRedirect('index.php?option=com_advancedtemplates&view=template&id=' . $templateID . '&file=' . $file);
		$model = $this->getModel('Template', 'AdvancedTemplatesModel');
		...
		JEventDispatcher::getInstance()->trigger(
			'onAfterTemplateCopy',
			['com_advancedtemplates.template', $fromName, $newName]
		);
	 ...
	 */
	public function getModel($name = 'Template', $prefix = 'AdvancedTemplatesModel', $config = [])
	{
	...
	public function renameFile()
	{
		...
		$rename = $model->renameFile($file, $newName);

		if ($rename)
		{
			$this->setMessage(JText::_('COM_TEMPLATES_FILE_RENAME_SUCCESS'));
			$url = 'index.php?option=com_advancedtemplates&view=template&id=' . $id . '&file=' . $rename;
			$this->setRedirect(JRoute::_($url, false));
	...
	public function save()
	{
		...
		if ($data['filename'] != end($explodeArray))
		{
			$app->enqueueMessage(JText::_('COM_TEMPLATES_ERROR_SOURCE_ID_FILENAME_MISMATCH'), 'error');
		...
		if ($data === false)
		{
			...
			// Redirect back to the edit screen.
			$url = 'index.php?option=com_advancedtemplates&view=template&id=' . $model->getState('extension.id') . '&file=' . $fileName;
			$this->setRedirect(JRoute::_($url, false));
	...
	public function uploadFile()
	{
		...
		$return = $model->uploadFile($upload, $location);

		if ($return)
		{
			$app->enqueueMessage(JText::_('COM_TEMPLATES_FILE_UPLOAD_SUCCESS') . $upload['name']);
			$redirect = base64_encode($return);
			$url      = 'index.php?option=com_advancedtemplates&view=template&id=' . $id . '&file=' . $redirect;
			$this->setRedirect(JRoute::_($url, false));
