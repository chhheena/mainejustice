...
class modSPTwitter{
	...
	*/		
	private function getData()
	{
		...
		if( empty($settings['consumer_key']) ){
			$app->enqueueMessage("Twitter Consumer Key not defined.", 'error');
			return false;
		} 			
		elseif( empty($settings['consumer_secret']) ){
			$app->enqueueMessage("Twitter Consumer secret not defined.", 'error');
			return false;
		} 
		elseif( empty($settings['oauth_access_token']) ){
			$app->enqueueMessage("Twitter access token not defined.", 'error');
			return false;
		} 
		elseif( empty($settings['oauth_access_token_secret']) ){
			$app->enqueueMessage("Twitter access token secret not defined.", 'error');
			return false;
		} 
	...
	*/	
	public function onError($params)
	{
		...
		if( isset( $data['errors'] ) or  isset( $data['error'] ) ) File::Delete($params['file']);
	}
